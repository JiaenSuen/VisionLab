...
from TrainingSection.dataset import ImageNette224Dataset, NUM_CLASSES_OF_FOOD101, DATASET_NAME
...
def Train(modelName="", device="cuda", epochs=100):
    ...
    train_loader = ImageNette224Dataset.GetTrainLoader(
        batch_size=config["batch_size"],
        augment=True,
    )

    # Data Loader for final train accuracy, without augmentation
    train_eval_loader = ImageNette224Dataset.GetTrainLoader(
        batch_size=config["test_batch_size"],
        augment=False,
    )

    # Test / validation loader
    test_loader = ImageNette224Dataset.GetTestLoader(
        batch_size=config["test_batch_size"],
    )
