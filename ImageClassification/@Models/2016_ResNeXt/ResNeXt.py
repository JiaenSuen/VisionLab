...
class ResNeXt29_CIFAR(nn.Module):
    def __init__(self, num_classes=10, cardinality=8, group_width_base=64):
        super().__init__()
        self.cardinality = cardinality
        self.group_width_base = group_width_base
        
        # CIFAR Stem (Paper Appendix A)
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1   = nn.BatchNorm2d(64)
        self.relu  = nn.ReLU(inplace=True)
        
        # 3 stages (only 3 blocks per stage -> 29 stages in total)
        self.layer1 = self._make_layer(64,  256, num_blocks=3, stride=1,  group_width=group_width_base)
        self.layer2 = self._make_layer(256, 512, num_blocks=3, stride=2,  group_width=group_width_base*2)
        self.layer3 = self._make_layer(512,1024, num_blocks=3, stride=2,  group_width=group_width_base*4)
        
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(1024, num_classes)   # Final stage out=1024
...
def resnext29_8x64d_cifar(num_classes=10):
    """32x32"""
    return ResNeXt29_CIFAR(num_classes=num_classes, cardinality=8, group_width_base=64)


if __name__ == "__main__":
    model_224 = resnext50_32x4d(num_classes=1000)
    model_32  = resnext29_8x64d_cifar(num_classes=10)
